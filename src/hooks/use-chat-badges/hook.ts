...
type BadgeResponses = [TwitchAPIBadgeResponse, TwitchAPIBadgeResponse];

export function useChatBadges(channelID: string): BadgeResponses {
  const [globalBadges, setGlobalBadges] = useState<TwitchAPIBadgeResponse>({
    badge_sets: {},
  });
  const [channelBadges, setChannelBadges] = useState<TwitchAPIBadgeResponse>({
    badge_sets: {},
  });
  ...
  useEffect(() => {
    twitchApiClient
      ...
      .catch((err) => console.error("Failed to get global badges", err));
  }, []);
